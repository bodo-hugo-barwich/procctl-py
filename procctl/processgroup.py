...
class ProcessGroup(object):
    ...
    def setCheckInterval(self, icheckinterval=-1):
        ...
        if self._check_interval > 0 \
            ...
            irdtmout = math.floor(self._check_interval /
                                  len(self._arr_commands))
    ...
    def Launch(self):
        ...
        if icmdcnt > 0:
            ...
            for icmdidx in range(0, icmdcnt):
                ...
                if cmd is not None:
                    scmdnm = "No. '{}' - {}".format(icmdidx,
                                                    cmd.getNameComplete())

                    stmnow = datetime.now().strftime('%F %T')

                    self._arr_rpt.append(
                        "{} : Sub Process {}: Launching ...\n".format(
                            stmnow, scmdnm))

                    # Launch the Sub Process through
                    # Process::SubProcess::Launch()
                    if cmd.Launch():
                        stmnow = datetime.now().strftime('%F %T')

                        self._arr_rpt.append(
                            "{} : Sub Process {}: Launch OK - PID ({})\n" .format(
                                stmnow, scmdnm, cmd.getProcessID()))
                        ...
                        self._arr_err.append(
                            "Sub Process {}: Launch failed!\nMessage: {}\n" .format(
                                scmdnm, cmd.error))
                ...
                if cmd is not None:
                    ...
                    if cmd.isRunning():
                        if cmd.Check():
                            ...
                            self._arr_rpt.append(
                                "{} : Sub Process {}: finished with [{}]\n" .format(
                                    stmnow, scmdnm, cmd.status))
                        ...
                        if cmd.getProcessID() > 0:
                            if self._bdebug:
                                self._arr_rpt.append(
                                    "Sub Process {}: already finished with [{}]\n" .format(
                                        scmdnm, cmd.status))
            ...
            if irng > 0:
                if self._check_interval > -1 \
                    ...
                    if self._execution_timeout > -1 \
                        ...
                        self._arr_err.append(
                            "Sub Processes 'Count: {}': Execution timed out!\n".format(irng))
                        self._arr_err.append(
                        ...
                        self.Terminate()
                        irng = -1
                    # if self._execution_timeout > -1 and itmrng >=
    ...
    def clearErrors(self):
        if self._bdebug:
            sdbgmsg = "'{}' : Call on '{}'\n" .format(
                sys._getframe(1).f_code.co_name, sys._getframe(0).f_code.co_name)
    ...
    def getFinishedCount(self):
        ...
        for cmd in self._arr_commands:
            if not cmd.isRunning() \
                ...
                # The Child Process was launched, has finished and was not
                # reset yet
                ifnshd += 0
