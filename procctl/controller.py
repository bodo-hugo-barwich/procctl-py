...
class ProcessController(object):
        ...
        if self._scommand != '':
            ...
            try:
                ...
                self._selector.register(
                    self._process.stdout, selectors.EVENT_READ, None)
                self._selector.register(
                    self._process.stderr, selectors.EVENT_READ, None)
                ...
                self._arr_err.append(
                    "Command '{}': Launch failed with {}!\n".format(
                        sprcnm, e.__class__.__name__))
                self._arr_err.append(
                    "Exception [{}] Message: {}\n".format(
                        e.errno, str(e)))
                self._err_code = 1
        ...
        if self._process is not None:
            if self._process.poll() is not None:
                ...
                if self._bdebug:
                    self._arr_rpt.append(
                        "prc ({}): finished with [{}].\n".format(
                            self._pid, self._process_status))
            ...
            if (irng > 0):
                if (self._execution_timeout > -1):
                    ...
                    if itmrng >= self._execution_timeout:
                        self._arr_err.append(
                            "Sub Process {}: Execution timed out!\n".format(sprcnm))
                        self._arr_err.append(
    ...
    def Terminate(self):
        ...
        if self.isRunning():
            self._arr_err.append(
                "Sub Process {}: Process terminating ...\n".format(sprcnm))
    ...
    def isRunning(self):
        ...
        # The Process got a Process ID but did not get a Process Status Code
        # yet
