...
def test_ExecutionTimeout():
    ...
    assert cmdtest.getExecutionTime(
    ) < itestpause, "Measured Time is greater or equal than the Full Run Time"
    ...
    if scripterror is not None:
        ...
        assert pat_tmout.search(
            scripterror) is not None, "STDERR does not report Execution Timeout"
...
def test_ScriptNotFound():
    ...
    if iscriptstatus == -1:
        assert not brunok, "script '{}': Execution did not fail".format(
            stestscript)
    else:
        assert not brunok, "script '{}': Execution did not fail".format(
            stestscript)
    ...
    if scripterror is not None:
        ...
        assert pat_ntfnd.search(
            scripterror) is not None, "STDERR does not report Not Found Error"
...
def test_NoPermission():
    ...
    if scripterror is not None:
        ...
        assert pat_noperm.search(
            scripterror) is not None, "STDERR does not report No Permission Error"
...
def test_BashError():
    ...
    if scripterror is not None:
        ...
        assert pat_synerr.search(
            scripterror) is not None, "STDERR does not report Bash Error"
...
def test_PythonException():
    ...
    if scripterror is not None:
        ...
        assert pat_except.search(
            scripterror) is not None, "STDERR does not report the Python Exeception"
